<?php

/**
 * Theme configuration file.
 *
 * @package capsule
 */

define('WP_CAPSULE_AUTHOR', 'Crowd Favorite');
define('WP_CAPSULE_THEME_NAME', 'Capsule');
define('WP_CAPSULE_THEME_SLUG', 'wp-capsule');
define('WP_CAPSULE_THEME_VERSION', '1.3.0');
define('WP_CAPSULE_DIR', get_template_directory());
define('WP_CAPSULE_URI', get_template_directory_uri());
define('WP_CAPSULE_UPDATER_DOWNLOAD_ID', '');
define('WP_CAPSULE_UPDATER_ITEM_ID', '41835');
define('WP_CAPSULE_UPDATER_REMOTE_API_URL', 'https://cfshopstage.wpengine.com/downloads/wp-capsule/');
define('WP_CAPSULE_UPDATER_RENEWAL_URL', '');
define('WP_CAPSULE_UPDATER_USE_BETA', false);
define('WP_CAPSULE_UPDATER_DIR', WP_CAPSULE_DIR . '/updater');
define('WP_CAPSULE_UPDATER_URI', WP_CAPSULE_URI . '/updater');
